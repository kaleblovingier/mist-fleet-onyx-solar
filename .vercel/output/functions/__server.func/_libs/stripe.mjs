...
var HttpClient = class HttpClient {
	...
	*/
	static makeResponseBodyError(exception) {
		...
		if (exception && typeof exception.code === "string") error.code = exception.code;
		return error;
	}
};
...
var HttpClientResponse = class {
	...
	_parseResponseBody(body) {
		try {
			...
			if (e instanceof Error) e.rawBody = body;
			throw e;
		}
	}
};
...
* parsing error.
*/
var HttpClientResponseBodyError = class extends Error {};
//#endregion
...
function flattenAndStringify(data) {
	...
	const step = (obj, prevKey) => {
		Object.entries(obj).forEach(([key, value]) => {
			...
			if (isObject(value)) {
				if (!(value instanceof Uint8Array) && !Object.prototype.hasOwnProperty.call(value, "data")) return step(value, newKey);
...
function validatePath(path) {
	if (typeof path !== "string" || !path.startsWith("/") || path.startsWith("//")) throw new Error(`Request path must be a string beginning with a single "/", got: ${JSON.stringify(path)}`);
}
/**
...
function maybeExtractFromCloudProviderEnvelope(payload) {
	...
	if ("detail" in parsed) return parsed.detail;
	if ("specversion" in parsed && "data" in parsed) return parsed.data;
	if (parsed.object === "event" || parsed.object === "v2.core.event") return parsed;
...
var RequestSender = class RequestSender {
	...
	*/
	_jsonResponseHandler(requestEvent, apiMode, usage, timeout, requestRetries, callback) {
		return (res) => {
			const headers = res.getHeaders();
			this._emitStripeNotice(headers);
			...
			res.toJSON().then((jsonResponse) => {
				...
				if (e instanceof HttpClientResponseBodyError && e.code === HttpClient.TIMEOUT_ERROR_CODE) throw RequestSender._generateConnectionError(e, timeout, requestRetries, requestId);
	...
	static _generateConnectionError(error, timeout, requestRetries, requestId) {
	...
	_defaultIdempotencyKey(method, apiMode) {
		const genKey = () => {
			let uuid;
			try {
	...
	_recordRequestMetrics(requestId, requestDurationMs, usage) {
		if (this._stripe.getTelemetryEnabled() && requestId) {
			if (this._stripe._prevRequestMetrics.length > this._maxBufferedRequestMetric) this._stripe._platformFunctions.emitWarning("Request metrics buffer is full, dropping telemetry message.");
			else {
				const m = {
	...
	_request(method, host, path, data, authenticator, options, usage = [], callback, requestDataProcessor = null) {
		...
		const makeRequest = (apiVersion, headers, numRetries) => {
			...
			authenticator(request).then(() => {
				...
				req.then((res) => {
					if (RequestSender._shouldRetry(res, requestRetries, maxRetries)) return retryRequest(makeRequest, apiVersion, headers, requestRetries);
					else if (options.streaming && res.getStatusCode() < 400) return this._streamingResponseHandler(requestEvent, usage, callback)(res);
					else return this._jsonResponseHandler(requestEvent, apiMode, usage, timeout, requestRetries, callback)(res);
				}).catch((error) => {
					if (error instanceof HttpClientRuntimeError) return callback(error);
					...
					else return callback(RequestSender._generateConnectionError(error, timeout, requestRetries));
				});
			}).catch((e) => {
...
var coerceV2RequestDiscriminatedUnion = (data, schema) => {
	if (typeof data !== "object" || Array.isArray(data)) return data;
	const discriminatorValue = data[schema.discriminator];
	if (typeof discriminatorValue !== "string") throw new Error(`Missing or invalid discriminator \`${schema.discriminator}\` for a polymorphic parameter. Stripe uses this field to determine the shape of the value, so we cannot encode the request without it. Provide \`${schema.discriminator}\` with one of: ${Object.keys(schema.variants).join(", ")}.`);
	const variant = schema.variants[discriminatorValue];
	return variant ? coerceV2RequestData(data, variant) : data;
};
var coerceV2RequestObject = (data, schema) => {
	if (typeof data !== "object" || Array.isArray(data)) return data;
	const obj = data;
	const result = {};
...
var coerceV2RequestData = (data, schema) => {
	...
	switch (schema.kind) {
		...
		case "object": return coerceV2RequestObject(data, schema);
		case "array":
		...
		case "discriminatedUnion": return coerceV2RequestDiscriminatedUnion(data, schema);
	}
};
var coerceV2ResponseDiscriminatedUnion = (data, schema) => {
	if (typeof data !== "object" || Array.isArray(data)) return data;
	const discriminatorValue = data[schema.discriminator];
	if (typeof discriminatorValue === "string" && discriminatorValue in schema.variants) return coerceV2ResponseData(data, schema.variants[discriminatorValue]);
	return data;
};
var coerceV2ResponseObject = (data, schema) => {
	if (typeof data !== "object" || Array.isArray(data)) return data;
	const obj = data;
...
var coerceV2ResponseData = (data, schema) => {
	...
	switch (schema.kind) {
		...
		case "decimal_string":
			...
			return data;
		case "object": return coerceV2ResponseObject(data, schema);
		case "array":
			if (!Array.isArray(data)) return data;
		...
		case "discriminatedUnion": return coerceV2ResponseDiscriminatedUnion(data, schema);
	}
};
//#endregion
...
var StripeResource = class {
	...
	_makeRequest(method, path, params, options, spec) {
		const requestMethod = method.toUpperCase();
		let data = (spec?.encode || ((data) => data))(params ? { ...params } : {});
		const processed = processOptions(options);
		...
		try {
			...
			if (spec?.requestSchema) data = coerceV2RequestData(data, spec.requestSchema);
		} catch (err) {
	...
	makeRequest(host, port, path, method, headers, requestData, protocol, timeout) {
		...
		return new Promise((resolve, reject) => {
			...
			req.setTimeout(timeout, () => {
				...
				if (res && !res.complete) res.destroy(timeoutError);
	...
	toJSON() {
		return new Promise((resolve, reject) => {
			...
			this._res.once("close", () => {
				if (!this._res.complete) reject(HttpClient.makeResponseBodyError(null));
			});
			this._res.once("end", () => {
				try {
...
var FetchHttpClient = class FetchHttpClient extends HttpClient {
	...
	static makeFetchWithRaceTimeout(fetchFn) {
		return async (url, init, timeout) => {
			...
			try {
				return {
					res: await Promise.race([fetchFn(url, init), timeoutPromise]),
					requestTimeout
				};
			} catch (err) {
				release();
	...
	async makeRequest(host, port, path, method, headers, requestData, protocol, timeout) {
		...
		const { res, requestTimeout } = await this._fetchFn(url.toString(), {
			method,
...
var FetchHttpClientResponse = class FetchHttpClientResponse extends HttpClientResponse {
	constructor(res, requestTimeout) {
...
var PlatformFunctions = class {
	...
	*/
	uuid4() {
		if (typeof crypto === "undefined" || !crypto.randomUUID) throw new Error("Stripe: no cryptographically secure random number generator is available in this runtime; `crypto.randomUUID` is required.");
...
var NodePlatformFunctions = class extends PlatformFunctions {
	...
	/** @override */
	emitWarning(warning) {
		if (typeof process.emitWarning === "function") process.emitWarning(warning, "Stripe");
		else super.emitWarning(warning);
	}
	/** @override */
	getEnv() {
		return process.env;
	...
	/** @override */
	getTelemetryId() {
		if (this._telemetryId !== void 0) return this._telemetryId;
		...
		const newId = crypto$1.randomBytes(16).toString("hex");
		try {
	...
	_getTelemetryIdPath() {
		...
		if (xdg) return path.join(xdg, "stripe", "telemetry_id");
...
function createWebhooks(platformFunctions) {
	function buildEvent(jsonPayload) {
		if (jsonPayload && jsonPayload.object === "v2.core.event") throw new Error("You passed a thin event notification to a function that expects a webhook. Use the corresponding EventNotification method instead.");
		return jsonPayload;
	}
	const Webhook = {
	...
	const signature = {
		...
		verifyHeader(encodedPayload, encodedHeader, secret, tolerance, cryptoProvider, receivedAt) {
			...
			if (!secret) throw new StripeSignatureVerificationError(header, payload, { message: "No webhook secret value was provided. It should start with `whsec_`" });
...
var ApiVersion = "2026-08-26.dahlia";
...
//#endregion
//#region node_modules/stripe/esm/resources/Billing/FeedbackOptions.js
var FeedbackOptionResource = class extends StripeResource {
	/**
	* An API method for listing the feedback options model
	*/
	list(params, options) {
		return this._makeRequest("GET", "/v1/billing/feedback_options", params, options, { methodType: "list" });
	}
	/**
	* Creates a new feedback option.
	*/
	create(params, options) {
		return this._makeRequest("POST", "/v1/billing/feedback_options", params, options);
	}
	/**
	* Retrieves a feedback options object given an ID.
	*/
	retrieve(id, params, options) {
		return this._makeRequest("GET", `/v1/billing/feedback_options/${encodeURIComponent(id)}`, params, options);
	}
	/**
	* Updates the description of an existing feedback option.
	*/
	update(id, params, options) {
		return this._makeRequest("POST", `/v1/billing/feedback_options/${encodeURIComponent(id)}`, params, options);
	}
	/**
	* Deactivates a feedback option. Deactivated feedback options cannot be used in portal configurations.
	*/
	deactivate(id, params, options) {
		return this._makeRequest("POST", `/v1/billing/feedback_options/${encodeURIComponent(id)}/deactivate`, params, options);
	}
};
//#endregion
...
//#endregion
//#region node_modules/stripe/esm/resources/V2/Commerce/ProductCatalog/Imports.js
...
var AccountResource = class extends StripeResource {
	...
	*/
	retrieve(id, params, options) {
		if (typeof id === "string") return this._makeRequest("GET", `/v1/accounts/${encodeURIComponent(id)}`, params, options);
...
var multipartDataGenerator = (data, headers, segmentBoundary) => {
	...
	for (const k in flattenedData) {
		...
		if (Object.prototype.hasOwnProperty.call(v, "data")) {
			const typedEntry = v;
			push(`Content-Disposition: form-data; name=${quote(k)}; filename=${quote(typedEntry.name || "blob")}`);
			push(`Content-Type: ${stripCrLf(typedEntry.type || "application/octet-stream")}`);
			push("");
...
function multipartRequestDataProcessor(method, data, headers, callback) {
	...
	this._stripe._platformFunctions.tryBufferData(data).then((bufferedData) => {
		return callback(null, multipartDataGenerator(bufferedData, headers, this._stripe._platformFunctions.uuid4()));
...
//#endregion
//#region node_modules/stripe/esm/StripeEventNotificationHandler.js
...
*/
var BaseEventNotificationHandler = class {
	constructor(client, fallbackCallback) {
		this.client = client;
		this.fallbackCallback = fallbackCallback;
		this.registeredHandlers = {};
		this.preHandleCallback = null;
		this.hasHandledEvent = false;
	}
	on(type, callback) {
		this.assertCanRegister();
		if (this.registeredHandlers[type]) throw new Error(`Callback for event type "${type}" is already registered.`);
	...
	*/
	assertCanRegister() {
		if (this.hasHandledEvent) throw new Error("Cannot register new callbacks after an event has been handled. This is indicative of a bug.");
	}
	/**
	...
	*/
	preHandle(callback) {
		this.assertCanRegister();
		if (this.preHandleCallback) throw new Error("A preHandle callback is already registered");
	...
	async dispatchEvent(event) {
		...
		if (this.preHandleCallback && !await this.preHandleCallback(event, eventClient)) return;
		const handler = this.registeredHandlers[event.type];
		if (handler) return await handler(event, eventClient);
		else return await this.fallbackCallback(event, eventClient, { isKnownEventType: KNOWN_EVENT_TYPES.has(event.type) });
	}
};
var StripeEventNotificationHandler = class extends BaseEventNotificationHandler {
	constructor(client, webhookSecret, fallbackCallback) {
		super(client, fallbackCallback);
		this.webhookSecret = webhookSecret;
		if (!webhookSecret) throw new Error("webhookSecret must be a non-empty string");
	}
	static withoutVerification(client, fallbackCallback) {
	...
	async handle(rawBody, signature) {
		...
		return await this.dispatchEvent(this.client.parseEventNotification(rawBody, signature, this.webhookSecret));
	}
};
/**
...
*/
var StripeEventNotificationHandlerWithoutVerification = class extends BaseEventNotificationHandler {
	async handle(rawBody) {
		this.hasHandledEvent = true;
		return await this.dispatchEvent(this.client.parseEventNotificationWithoutVerification(rawBody));
	}
};
//#endregion
...
//#endregion
//#region node_modules/stripe/esm/resources/V2/Commerce/ProductCatalog/index.js
...
//#endregion
//#region node_modules/stripe/esm/resources/V2/Commerce/index.js
var Commerce = class {
	constructor(stripe) {
		this.stripe = stripe;
...
var Stripe = class Stripe {
	static initialize(platformFunctions, requestSenderFactory = defaultRequestSenderFactory) {
		...
		if (env?.CLAUDECODE || env?.CLAUDE_CODE_CHILD_SESSION) try {
	...
	*/
	...
	_buildEventNotification(parsed) {
		if (parsed.context) parsed.context = StripeContext.parse(parsed.context);
		parsed.fetchEvent = () => {
			return this._requestSender._rawRequest("GET", `/v2/core/events/${encodeURIComponent(parsed.id)}`, void 0, {
				stripeContext: parsed.context,
				headers: { "Stripe-Request-Trigger": `event=${parsed.id}` }
			}, ["fetch_event"]);
		};
		parsed.fetchRelatedObject = () => {
			if (!parsed.related_object) return Promise.resolve(null);
	...
	parseEventNotification(payload, header, secret, tolerance, cryptoProvider, receivedAt) {
		...
		if (eventNotification.object === "event") throw new Error("You passed a webhook payload to stripe.parseEventNotification, which expects an event notification. Use stripe.webhooks.constructEvent instead.");
		if (eventNotification.object !== "v2.core.event") throw new Error(`Unexpected object type '${eventNotification.object}'. Expected 'v2.core.event' for an event notification.`);
	...
	*/
	parseEventNotificationWithoutVerification(payload) {
		...
		if (inner.object === "event") throw new Error("It looks like this cloud event contains a webhook body instead of a thin event notification. Use constructEventWithoutVerification instead.");
		if (inner.object !== "v2.core.event") throw new Error(`Unexpected object type '${inner.object}'. Expected 'v2.core.event' for an event notification.`);
	...
	notificationHandlerWithoutVerification(fallbackCallback) {
		return StripeEventNotificationHandler.withoutVerification(this, fallbackCallback);
	}
};
Stripe.PACKAGE_VERSION = "22.6.2";
