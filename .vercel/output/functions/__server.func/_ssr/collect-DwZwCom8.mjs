...
//#region node_modules/.nitro/vite/services/ssr/assets/collect-DwZwCom8.js
...
var collectLicenses = createServerFn({ method: "POST" }).validator((input) => ({
	...
	const { collectLicenses: run } = await import("./collect.server-BG5wQOVw.mjs");
	return run(data);
});
...
var draftCollected = createServerFn({ method: "POST" }).validator((input) => ({
	...
	const { draftCollected: run } = await import("./collect.server-BG5wQOVw.mjs");
	return run(data);
});
//#endregion
