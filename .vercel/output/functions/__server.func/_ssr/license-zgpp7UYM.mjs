...
//#region node_modules/.nitro/vite/services/ssr/assets/license-zgpp7UYM.js
...
var redeemLicense = createServerFn({ method: "POST" }).validator((input) => ({ key: readString(input, "key") })).handler(redeemLicense_createServerFn_handler, async ({ data }) => {
	const { verifyKey } = await import("./license.server-CxeknnL7.mjs");
...
var mintLicenseKey = createServerFn({ method: "POST" }).validator((input) => ({
	...
	const { mintKey, mintKeyFromNote, pinOk, defaultPinSet, parseIssuedPlan } = await import("./license.server-CxeknnL7.mjs");
...
var mintLicenseBatch = createServerFn({ method: "POST" }).validator((input) => ({
	...
	const { mintKeyedRows, parseIssuedPlan, parseNameList, pinOk, defaultPinSet } = await import("./license.server-CxeknnL7.mjs");
	if (!pinOk(data.pin)) return {
		ok: false,
		reason: "Operator PIN is wrong."
	};
